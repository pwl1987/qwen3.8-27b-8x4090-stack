...
## 十、方向 A 根因闭环（2026-09-08，P47/P48/P49 的续案）

- **P60 残差"损坏"实为引擎级布局非确定性（P48 定性翻转）**：LOOKUP=0 下矩阵 **7/12** 比 L1 的
  3~4/12 更重——与 lookup/adaptive 无关。根因：暖命中轮尾段以恢复点为新 chunk 边界重算 ≠ 冷轮
  分块边界 → 混合 GDN 状态扫描比特差 → 近平局 greedy 翻转。全部分歧为**连贯备选续写**（无垃圾/
  复读）→ 良性类。token 级证据（容器内 trace 插桩）：b2 恢复 nct=8704/10514，首分歧在生成区第
  41 token，两轮 verify 形状恒等。上游 #55524 exact-replay RFC 明确排除 spec+prefix 组合的位一致
  → 本栈无解，只能接受并监测（`multi_residue_test` 语义改分歧率监测）。连带发现：**现产 130.0
  （L0+PC1）本身就带 7/12 暖/冷分歧**（无用户流量，上产须知单轮零影响/多轮用 cache_salt）。
- **P61 "171.6 档"是 per-boot 双峰采样（P47/P49 增益不可交付的真相）**：相同配置与流程下 t3 呈
  ~169（41.3%/tok）或 ~120（27.1%/tok）二选一，boot 内粘滞、跨 boot 随机（合并 ~44% 高），
  低档**低于现产 130**；g512 先行/完整 canary 补救全无效；步速正常纯接受率塌缩（t3 刀刃内容
  翻到非引用变体 → lookup 引用链断）。**方法论教训：t3 类刀刃夹具单 boot 数字不可作配置结论，
  重要 A/B 至少 3 个独立容器实例。**
- **P62 adaptive 在 k=7 结构性失效**：`draft_block == num_speculative_steps == 7` → adaptive
  分支永不进入，8↔16 块长切换（start_qwen.sh 自测矩阵的场景）在生产候选配置上不存在；A8D-ADAPT0
  env 为 no-op，其 122.6 是低档 boot——历史单臂结论要过"env 是否真的有效"这一关。
  结案文档 `eval/vllm/p0/S4-ROOTCAUSE-20260908.md` + `evidence-20260908/`。

## 十一、通用教训（跨期提炼）

1. **口径纪律**：任何性能数字必须钉死 harness / 功耗档 / 引擎暖态 / 计数器语义——四个都坑过。
2. **单变量 + 全新 recreate**：配置差一个字母的结论不合并。
3. **质量一票否决**：速度增益无论多大，确定性输出损坏即否决（P48/P37；09-08 复核：P48 定性
   翻转为布局非确定性良性分歧，真损坏判据=垃圾/复读/乱码，见 P60）。
