...
class _LinearAttentionVReorderBase(Qwen3NextModel):
    ...
    def _reorder_v_heads(tensor: Tensor, dim: int, num_k_heads: int, num_v_per_k: int, head_dim: int) -> Tensor:
        ...
        # LoRA adapter tensors (from convert_lora_to_gguf) cannot go through the
        # reshape/permute gymnastics below; for them the reorder is a pure index
        # permutation applied to the owning factor matrix (rows -> B, cols -> A).
        if hasattr(tensor, 'get_lora_A_B'):
            n = tensor.shape[dim]
            perm = _LinearAttentionVReorderBase._reorder_v_heads(
                torch.arange(n), 0, num_k_heads, num_v_per_k, head_dim)
            return torch.index_select(tensor, dim, perm)
