    ...
    def __torch_function__(cls, func: Callable, types, args=(), kwargs=None):
        ...
        if func is torch.permute:
            ...
            if len(args[0][0].shape) > 2:
                ...
                raise NotImplementedError
        elif func is torch.index_select:
            assert len(args) >= 3
            tensor, dim, index = args[0], args[1], args[2]
            shape = tensor.shape
            if dim < 0:
                dim += len(shape)
            assert len(shape) == 2, 'LoraTorchTensor index_select only supported for 2D tensors'
            if dim == 0:
                # row permutation: reorder B's rows, A unchanged
                return LoraTorchTensor(tensor._lora_A, tensor._lora_B.index_select(0, index))
            else:
                # column permutation: reorder A's columns, B unchanged
                return LoraTorchTensor(tensor._lora_A.index_select(1, index), tensor._lora_B)
