...
EOF
command -v nvidia-smi >/dev/null && { PL=$(nvidia-smi --query-gpu=power.limit --format=csv,noheader,nounits | head -1); ok "power limit ${PL} W (README numbers are at 280 W)"; }
fi
...
def fail(m):
    global F
    print("  FAIL ", m); F += 1
# The standard prepared model uses int8; the single-user fast variant uses a
# separately calibrated int4 GPTQ head. Both are valid packed replacements for
# the original BF16 head and both are described by the matching config group.
head_bits = {
    g["weights"]["num_bits"]
    for g in groups.values()
    if g["targets"] == ["re:.*lm_head$"] and g.get("weights")
}
if "lm_head.weight_packed" in idx and head_bits & {4, 8}:
    ok(f"lm_head requantized to int{min(head_bits & {4, 8})}")
else:
    fail("lm_head not requantized: run prepare/quant_lm_head.py or use a qualified fast variant")
...
FAST_MODEL=${FAST_MODEL:-$HERE/models/Qwen3.8-27B-W4A16-AutoRound-fast}
case "$(basename "$MODEL")" in *-fast) FAST_MODEL=$MODEL ;; esac
if [ -d "$FAST_MODEL" ]; then ok "fast variant present at $FAST_MODEL (int4-GPTQ lm_head/MTP, own-output draft vocab)"; else warn "no fast variant (venv/bin/python prepare/fetch_fast_variant.py; single-user mode is ~15% slower without it)"; fi
...
if [ $NOSRV = 0 ]; then
  echo "== live server (127.0.0.1:${PORT:-19622})"
  PORT=${PORT:-19622}
