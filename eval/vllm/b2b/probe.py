#!/usr/bin/env python3
"""B2-B arm probe (tier-2 screen): 16 FINAL prompts (the B2-A set, all inside
the B1-C A/B cache for paired comparison) + t3 double-run contract, against
one sandbox boot.  Usage: probe.py <label>"""
import json
import sys
...
rows = [{"target_tokens": 512, "tok_per_step": t3a["tok_per_step"],
         "sha12": t3a["sha12"], "drafts": 0, "accepted": 0,
         "chars": len(t3a["text"])}] + rows
out = {
    "label": label,
...
with open(f"/tmp/b2b/probe-{label}.json", "w") as f:
    json.dump(out, f, indent=1, ensure_ascii=False)
ok = [r for r in rows if r.get("tok_per_step")]
print(f"[{label}] {len(ok)}/{len(rows)} valid, "
      f"det={out['contract']['self_deterministic']}, "
      f"mean tok/step={sum(r['tok_per_step'] for r in ok)/len(ok):.4f} "
      f"sha={t3a['sha12']}")
